...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        public static void convertMoney(User user){
            ...
            try {
                ...
                if(OptionalFrom.isEmpty()) {
                    System.out.println("One of wallets (From) is missing. Can not convert.");
                    return;
                }

                Wallet to = findUserWallet(user, toCurrency).orElseThrow(()-> new IllegalArgumentException("Wallet (to) not found"));

                Wallet from = OptionalFrom.get();
        ...
        public static Optional<Wallet> findUserWallet(User user, String currency) {
           ...
           try{
               Wallet wallet = em.createQuery(query).getSingleResult();
        ...
        //Використані змішані підходи для демонстрації

        Wallet walletUSD = findUserWallet(user, "usd").orElse(null);
       Optional<Wallet> walletOptimalEUR = findUserWallet(user, "eur");
       Optional<Wallet> walletOptimalUAN = findUserWallet(user, "uah");


            if(walletUSD != null && walletOptimalEUR.isPresent() && walletOptimalUAN.isPresent()) {

                Wallet walletEUR = walletOptimalEUR.get();
                Wallet walletUAN = walletOptimalUAN.get();

                Double amountFromUSD = convertAmount(walletUSD, walletUAN, walletUSD.getAmount());
                Double amountFromEUR = convertAmount(walletEUR, walletUAN, walletEUR.getAmount());

                Double amountResult = amountFromUSD + amountFromEUR + walletUAN.getAmount();
